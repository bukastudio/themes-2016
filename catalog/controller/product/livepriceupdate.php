...
class ControllerProductLivepriceupdate extends Controller {
	...
	public $options_container 			= '#main-content'; 			// in default them it is 			"#main-content"
				...
				if (isset($this->request->post['option']) && $this->request->post['option']) {
                    foreach ($this->model_catalog_product->getProductOptions($product_id) as $option) {
                        foreach ($option['option_value'] as $option_value) {
                            //If options select and radio button
                            if(isset($this->request->post['option'][$option['product_option_id']]) && !is_array($this->request->post['option'][$option['product_option_id']])) {
                               if ($this->request->post['option'][$option['product_option_id']] == $option_value['product_option_value_id']) {
		...
		$js = <<<HTML
			...
			var livepriceupdate = function() {
				$.ajax({
					...
					data: jQuery('{$this->options_container} input[type=\'text\'], {$this->options_container} input[type=\'hidden\'], {$this->	options_container} input[type=\'radio\']:checked, {$this->options_container} input[type=\'checkbox\']:checked, {$this->options_container} select, {$this->options_container} textarea'),
					dataType: 'json',
						beforeSend: function() {
						...
						if (json.success) {
							if (jQuery('{$this->options_container} {$this->tax_price_container}').length > 0 && json.new_price.tax) {
																	animation_on_change_price_with_options('{$this->options_container} {$this->tax_price_container}', json.new_price.tax);
																}
																if (jQuery('{$this->options_container} {$this->old_price_container}').length > 0 && json.new_price.price) {
																	animation_on_change_price_with_options('{$this->options_container} {$this->old_price_container}', json.new_price.price);
																}
																if (jQuery('{$this->options_container} {$this->special_price_container}').length > 0 && json.new_price.special) {
																	animation_on_change_price_with_options('{$this->options_container} {$this->special_price_container}', json.new_price.special);
																}
																if (jQuery('{$this->options_container} {$this->price_container}').length > 0 && json.new_price.price) {
			...
			{
				jQuery('{$this->options_container} input[type=\'text\'], {$this->options_container} input[type=\'hidden\'], {$this->options_container} input[type=\'radio\']:checked, {$this->options_container} input[type=\'checkbox\'], {$this->options_container} select, {$this->options_container} textarea, #main-content input[name=\'quantity\']').live('change', function() {
						livepriceupdate();
					});
			}
			else
