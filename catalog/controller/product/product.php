...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			//$this->document->addScript('catalog/view/javascript/jquery/jquery.flexslider-min.js');
			//$this->document->addStyle('catalog/view/javascript/jquery/colorbox/colorbox.css');
      ...
      // Out of stock for option
      $this->language->get('text_option_stock');
      // End
      $this->data['text_sizechart'] = $this->language->get('text_sizechart'); // Size Chart
      ...
      $this->data['sizechart'] =  $this->model_catalog_product->getSizechartDetail($product_info['sizechart']); // Size Chart
			...
			$weight = $this->weight->format($weight,$this->config->get('config_weight_class_id'), $this->language->get('decimal_point'), $this->language->get('thousand_point'));
			$this->data['weight'] = $weight;
			$this->data['config_display_option_value']       = $this->config->get('config_display_option_value');
      $this->data['config_attributes_on_product_page']       = $this->config->get('config_attributes_on_product_page'); // Bukatoko
			...
			$length_class = $length_class_query->row['unit'];
			//$this->data['dimension'] = "{$length} x {$width} x {$height}{$length_class} <span>(LxWxH){$length_class}</span>";
      $this->data['dimension'] = "{$length} x {$width} x {$height}{$length_class}";
			// End
      ...
      $this->data['prod_name'] = $product_info['name'];
			...
			if ($product_info['quantity'] <= 0) {
				...
				// Bukatoko
				$this->data['stock_out'] = 'yes';
			} elseif ($this->config->get('config_stock_display')) {
				$this->data['stock'] = "Qty: " . $product_info['quantity'];
				// Bukatoko
				$this->data['stock_out'] = '';
			} else {
			...
			foreach ($results as $result) {
				$this->data['images'][] = array(
					...
					// Bukatoko
					 'tiny' => $this->model_tool_image->resize($result['image'], $this->config->get('config_image_additional_width'), $this->config->get('config_image_additional_height'), 'watermark')
				);
			}
			...
			if ((float)$product_info['special']) {
				...
				$this->data['price_discount']  = number_format((100 - ($product_info['special'] / $product_info['price']) * 100),0);
        $special = $this->model_catalog_product->getProductSpecialData($this->request->get['product_id']);
  			if($special['date_end'] && $this->config->get('countdowntimer_product') && time() < strtotime($special['date_end'])){
  				...
  				$this->data['special_end'] = $special['date_end'];
  				...
  				$this->data['special_end'] = false;
  			}
			} else {
				$this->data['special'] = false;
				$this->data['price_discount'] = false;
        $this->data['special_end'] = false;
			}
			...
			foreach ($discounts as $discount) {
				$this->data['discounts'][] = array(
					...
					// Bukatoko
					'margin' => round(((($product_info['price'] - $discount['price']) / $product_info['price']) *100),2) ,
			...
			foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id']) as $option) {
				if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') {
					...
					foreach ($option['option_value'] as $option_value) {
						//if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) { // Out of stock for option
              if (!$option_value['subtract'] || is_numeric($option_value['quantity'])) {
                ...
                'quantity'                => $option_value['quantity'], // Out of stock for option
