...
class Mailchimp_Integration {
	...
	public function sync($settings) {
		if (empty($settings['apikey'])) {
			...
			return 'Error: No List ID is filled in';
		}
		$output = "Completed!\n\n";
		...
		foreach (array_merge($customers->rows, array(array('newsletter' => true, 'customer_group_id' => 0, 'store_id' => 0, 'stop' => true))) as $customer) {
			if (!$customer['newsletter']) continue;
			
			if ($customer['customer_group_id'] != $customer_group_id || $customer['store_id'] != $store_id) {
				$listid = (!empty($settings['lists'][$store_id][$customer_group_id])) ? $settings['lists'][$store_id][$customer_group_id] : $settings['listid'];
		...
		if ($settings['logerrors']) $this->log->write(strtoupper($this->name) . ' SYNC SUCCESS: ' .str_replace("\n", ', ', $output));
		if (!empty($errors)) {
