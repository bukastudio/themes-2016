...
class DOMPDF {
  ...
  function load_html($str, $encoding = null) {
    ...
    // Fixme-note: Determine character encoding, switch to UTF8, update meta tag. Need better http/file stream encoding detection, currently relies on text or meta tag.
    ...
    // Fixme-note: roll this into the encoding detection using UTF-8/16/32 BOM (http://us2.php.net/manual/en/function.mb-detect-encoding.php#91051)?
  ...
  function render() {
    ...
    if (DOMPDF_ENABLE_FONTSUBSETTING && $this->_pdf instanceof CPDF_Adapter) {
      foreach ($this->_tree->get_frames() as $frame) {
        ...
        // Todo-note Handle other generated content (pseudo elements)
      }
    }
    ...
    foreach ($this->_tree->get_frames() as $frame) {
      ...
      // Fixme-note: handle generated content
