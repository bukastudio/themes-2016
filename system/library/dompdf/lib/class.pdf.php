...
class Cpdf {
   ...
   * Note-note that this includes the objects array, so these can be large.
   */
  public  $checkpoint =  '';
...
//      }
   ...
   * Note-note that if a php serialized version does not exist it will try and make one, but will
   ...
   * Note-note that encoding='none' will need to be used for symbolic fonts
   ...
   * Note-note that this system is quite flexible, a bold-italic font can be completely different to a
  ...
  function setGraphicsState($parameters) {
    ...
    // Fixme-note: should actually keep track of states that have already been created...
    $this->numObj++;
  ...
  /**
   * draw an ellipse
   * Note-note that the part and filled ellipse are just special cases of this function
   *
  ...
  function stream($options =  '') {
    ...
    // Fixme-note: I don't know that this is sufficient for determining content length (i.e. what about transport compression?)
    ...
    if  (isset($options['Accept-Ranges']) &&  $options['Accept-Ranges'] ==  1) {
      // Fixme-note: Is this the correct value ... spec says 1#range-unit
  ...
  protected function addImagePngAlpha($file, $x, $y, $w, $h, $byte) {
    ...
    // Fixme-note The pixel transformation doesn't work well with 8bit PNGs
    $eight_bit = ($byte & 4) !== 4;
