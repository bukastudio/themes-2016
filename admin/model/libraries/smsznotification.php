...
class ModelLibrariesSmszNotification extends Model {
	...
	public function get_balance($gateway) {
		$this->language->load('module/sendsmsznotification');
	    ...
	    mb_http_output("UTF-8");
			...
			$xmldata = new SimpleXMLElement($getresponse);
			$status = $xmldata->message[0]->text;
			$status = $status.",".$xmldata->message[0]->activedate;
		}elseif( $gateway == "oneway" ){
			$userkey = $this->config->get('userkey_oneway');
			$passkey = $this->config->get('passkey_oneway');
			$url = "http://reguler.sms-notifikasi.com/apps/smsapibalance.php";
			$content = 'userkey='.rawurlencode($userkey).
					       '&passkey='.rawurlencode($passkey);
			mb_internal_encoding("UTF-8");
	    mb_http_output("UTF-8");
			...
			curl_close($ch);
			$xmldata = new SimpleXMLElement($getresponse);
			$status = $xmldata->message[0]->value;
			$status = $status.",".$xmldata->message[0]->text;
		}elseif( $gateway == "twoway" ){
			$userkey = $this->config->get('userkey_twoway');
			$passkey = $this->config->get('passkey_twoway');
			$url = "http://".$this->config->get('httpapi').".sms-notifikasi.com/api/credit.php";
			$content = 'userkey='.rawurlencode($userkey).
					       '&passkey='.rawurlencode($passkey);
			mb_internal_encoding("UTF-8");
	    mb_http_output("UTF-8");
			...
			curl_close($ch);
			$xmldata = new SimpleXMLElement($getresponse);
			$status = $xmldata->credit[0]->value;
			$status = $status.",".$xmldata->credit[0]->activedate;
		}else{
			$status = $this->language->get('error_gateway_null');
		}
