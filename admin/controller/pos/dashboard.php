...
class ControllerPosDashboard extends Controller {
	private $error = array(); 
        ...
        public function resetCounters(){
            $this->db->query('update '.DB_PREFIX.'user set cash=0, card=0');
            $this->redirect($this->url->link('pos/dashboard/', 'token=' . $this->session->data['token'], 'SSL'));
        }
        ...
        public function index(){
            ...
            foreach ($rows as $row){                
                $row['total_cash'] = $this->model_pos_pos->get_today_cash($row['user_id']);
                $row['total_card'] = $this->model_pos_pos->get_today_card($row['user_id']);
                $this->data['rows'][] = $row;
            }
        ...
        public function history(){
            ...
            $pagination->url = $this->url->link('pos/dashboard/history&user_id='.$user_id, 'token=' . $this->session->data['token'] . '&page={page}', 'SSL');
