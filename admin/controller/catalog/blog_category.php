...
class ControllerCatalogBlogCategory extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect($this->url->link('catalog/blog_category', 'token=' . $this->session->data['token'], 'SSL'));
		}
	...
	private function getList() {
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('catalog/blog_category', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);

		$this->data['insert'] = $this->url->link('catalog/blog_category/insert', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['delete'] = $this->url->link('catalog/blog_category/delete', 'token=' . $this->session->data['token'], 'SSL');

		$this->data['categories'] = array();

		$results = $this->model_catalog_blog_category->getBlogCategories(0);
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('catalog/blog_category/update', 'token=' . $this->session->data['token'] . '&blog_category_id=' . $result['blog_category_id'], 'SSL')
			);
	...
	private function getForm() {
		...
		if (!isset($this->request->get['blog_category_id'])) {
			$this->data['action'] = $this->url->link('catalog/blog_category/insert', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$this->data['action'] = $this->url->link('catalog/blog_category/update', 'token=' . $this->session->data['token'] . '&blog_category_id=' . $this->request->get['blog_category_id'], 'SSL');
		}

		$this->data['cancel'] = $this->url->link('catalog/blog_category', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['blog_category_description'])) {
			...
			$this->data['blog_category_description'] = $this->model_catalog_blog_category->getBlogCategoryDescriptions($this->request->get['blog_category_id']);
		} else {
		...
		if (isset($this->request->post['blog_category_store'])) {
			...
			$this->data['blog_category_store'] = $this->model_catalog_blog_category->getBlogCategoryStores($this->request->get['blog_category_id']);
		} else {
		...
		if (isset($blog_category_info) && $blog_category_info['image'] && file_exists(DIR_IMAGE . $blog_category_info['image'])) {
