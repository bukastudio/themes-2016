...
abstract class elFinderVolumeDriver {
	...
	public function mount(array $opts) {
		...
		$this->tmbSize  = $this->options['tmbSize'] > 0 ? (int)$this->options['tmbSize'] : 80;
	...
	protected function encode($path) {
		if ($path !== '') {
			...
			// Todo-note crypt path and return hash
			$hash = $this->crypt($p);
	...
	protected function decode($hash) {
		if (strpos($hash, $this->id) === 0) {
			...
			// Todo-note uncrypt hash and return path
			$path = $this->uncrypt($h);
